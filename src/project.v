...
//-- - - - - - - - - - - - - - - - - - - - - - - - --
//-- Simplified-Heart-ms-sys-32bits: Nominal periodic signal in mili seconds
//---------------------------------------------------
//---- Top entity
module vd30ca9 (
 output v9fb85f
...
//-- Unit-ms-32bits
//-- - - - - - - - - - - - - - - - - - - - - - - - --
//-- Unit-ms-32bits: ms to cycles converter
//---------------------------------------------------
...
module main_v3d4960 (
 ...
 reg [7:0] cnt;          //variable zum hochzählen
 reg [7:0] data_out;     //variable als Ausgang
 
 reg [0:0] var_disp_sel; //variable, welches Display an sein soll
 reg [7:0] cnt_high;     //speichert die 10er Stelle des cnt
 reg [7:0] cnt_low;      //speichert die 1er stelle des cnt
 
 assign OUT = data_out;  // Verknüpfen der Ausgangsvariable mit Ausgang
 
 // ein always block zum zählen bei jeder positiven Flanke der clock
 always @(posedge clock) begin
     if (cnt >= 99)      // wenn der Counter größer als 99 ist
         cnt <= 0;      // setze ihn auf 0
 
     if (cnt < 99)       // wenn er kleinr als 99 ist
         cnt <= cnt +1;  //addiere 1 dazu
 end    
 
 
 // 7 Segment als Bitfolge:
 // Segment ist invertiert. 0 an, 1 aus.
 //    ---1---
 //    |     |
 //    6     2
 //    |     |
 ...
 always @(posedge disp_sel) begin        //bei positiver Flanke
     var_disp_sel <= ~var_disp_sel;      //invertiere die variable und ändere so das display
     cnt_high <= cnt / 10;               // teile cnt durch 10 OHNE REST (z.b 23 / 10 = 2)
     cnt_low <= cnt % 10;                // teile cnt durch 10 NUR REST (z.b 23 & 10 = 3)
     
     if (var_disp_sel == 1) begin        //wenn hinteres display an sein soll
         case (cnt_low)                  //prüfe cnt_low und setze den ausgang entsprechend
     ...
     if (var_disp_sel == 0) begin        //wenn vorderes display an sein soll
         case (cnt_high)                 //prüfe cnt_high und setze den ausgang entsprechend
             0: data_out = 8'b00000010;
             1: data_out = 8'b10011110;
             2: data_out = 8'b00100100;
             3: data_out = 8'b00001100;
     ...
     data_out[0] = var_disp_sel;         // setze das Bit entsprechend dem Display!!
